...
| --- | --- | --- |
| Colors (brand, text, backgrounds, status colors) | `src/styles/tokens.css` | Every color has a semantic name like `--color-primary`. Use the name, never a hex code, anywhere else. |
...
| Dark mode | `[data-theme="dark"]` block at the bottom of `tokens.css` | Unused today. Add `data-theme="dark"` to `<html>` to try it. |
...
- [ ] Ask whether dark mode is wanted. If not, delete the `[data-theme="dark"]` block.
...
- [ ] Map each Figma color to a token. Keep the *names* (`--color-primary`, `--color-surface`…) and change the *values*. Add new tokens only if the design has a color with no equivalent.
...
- [ ] Remove sections the design does not have (for example Testimonials).

**F. Copy**

- [ ] Replace placeholder text in `src/content/index.ts`: site name, tagline, description, phone, email, address, hours, about text, testimonials, terms of service.
...
| --- | --- | --- | --- |
| Home | `/` | `src/app/(public)/page.tsx` | Hero, quick search (location + dates), featured vehicles, how it works, testimonials, call to action |
| Vehicles | `/vehicles` | `src/app/(public)/vehicles/page.tsx` | Catalog with filters (category, transmission, price, seats), sorting, pagination, and a trip summary when dates are given |
| Vehicle detail | `/vehicles/[slug]` | `src/app/(public)/vehicles/[slug]/page.tsx` | Gallery, description, specs, features, price and Book now |
...
| Booking, step 1 to 6 | `/book/dates`, `/book/vehicle`, `/book/details`, `/book/payment`, `/book/review`, `/book/confirmation/[reference]` | `src/app/(public)/book/**` | Dates and place, vehicle and extras, driver details, mock payment, review and confirm, confirmation |
...
| Navigation | `Navbar`, `Footer`, `AdminShell` (sidebar plus top bar), `Pagination`, `Stepper` (booking progress), `SkipLink` |
...
| Search | `QuickSearch` |
| Booking | `BookingShell`, `DatesStep`, `VehicleStep`, `VehicleOption`, `DetailsStep`, `PaymentStep`, `ReviewStep`, `Confirmation`, `BookingSummary`, plus small parts in `step-parts.tsx` |
| Contact | `ContactForm` |
...
- **Payment step:** the site never asks for card numbers. With a real payment company, card details are typed into the company's own secure fields or page. The design should leave room for that (an embedded box or a redirect). The Card / GCash / Maya / Pay at pick-up choices are radio options.
